...
class PowerMaxAlarm : public uart::UARTDevice, public Component {
 public:
   ...
   *
   * ```cpp
   * class MyPowerMaxAlarm : public Component, public mqtt:PowerMaxAlarm {
   *  public:
   *   void setup() override {
   *     subscribe("the/topic", &MyPowerMaxAlarm::on_message);
   *     pinMode(5, OUTPUT);
   *   }
   *
   ...
   *  public:
   *   void setup() override {
   *     subscribe_json("the/topic", &MyPowerMaxAlarm::on_json_message);
   *     pinMode(5, OUTPUT);
   *   }
   *
...
template<typename T>
void PowerMaxAlarm::subscribe(const std::string &topic,
...
template<typename T>
void PowerMaxAlarm::subscribe(const std::string &topic, void (T::*callback)(const std::string &), uint8_t qos) {
  auto f = std::bind(callback, (T *) this, std::placeholders::_2);
  global_mqtt_client->subscribe(topic, f, qos);
}
template<typename T> void PowerMaxAlarm::subscribe(const std::string &topic, void (T::*callback)(), uint8_t qos) {
  auto f = std::bind(callback, (T *) this);
  global_mqtt_client->subscribe(topic, f, qos);
}
template<typename T>
void PowerMaxAlarm::subscribe_json(const std::string &topic, void (T::*callback)(const std::string &, JsonObject &),
                                      uint8_t qos) {
  auto f = std::bind(callback, (T *) this, std::placeholders::_1, std::placeholders::_2);
  global_mqtt_client->subscribe_json(topic, f, qos);
}
template<typename T>
void PowerMaxAlarm::subscribe_json(const std::string &topic, void (T::*callback)(JsonObject &), uint8_t qos) {
  auto f = std::bind(callback, (T *) this, std::placeholders::_2);
  global_mqtt_client->subscribe_json(topic, f, qos);
}
